...
//
// `/bangbwayo` 메인과 동일한 셋 카드 디자인 — 시간 라벨, 썸네일, 트랙 라벨
// 미리보기, 응답 분포 도트, 상태 칩 모두 한 번의 시각.
// 라우팅: 활성 셋 = 트랙 목록, 그 외 = 결과물 페이지
    ...
    <Card tok={tok} padding={0} overflow="hidden" style={{ marginBottom: 16 }}>
      ...
      {cards.map((c, i) => (
        <div key={c.id} style={{
          ...
          <SetCardRow tok={tok} summary={c} href={hrefFor(c.status, c.id)} />
        </div>
      ))}
    </Card>
  )
