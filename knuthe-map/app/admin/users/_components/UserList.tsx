...
const ROLE_STYLE: Record<string, { bg: string; color: string; label: string }> = {
  tenant: { bg: 'var(--border-primary)', color: 'var(--text-secondary)', label: '학생' },
  owner:  { bg: '#f0fdf4', color: 'var(--color-green)', label: '건물주' },
  agent:  { bg: '#f5f3ff', color: 'var(--color-purple)', label: '중개사' },
      ...
      <div style={{ display: 'flex', gap: 6, background: 'var(--bg-tertiary)', borderRadius: 10, padding: 4 }}>
        {(['all', ...ROLE_OPTIONS] as const).map((r) => (
          <button key={r} onClick={() => setRoleFilter(r)} style={{
            ...
            background: roleFilter === r ? 'var(--bg-elevated)' : 'transparent',
