...
export default function OnboardingClient() {
  ...
  const handleSwipeComplete = (weights: SwipeWeights) => {
    ...
    // 학과·학번도 함께 저장: OAuth callback 의 prefs 동기화 분기가 users.dept/grade 를
    // 채워주는 백업 경로로 동작 (메인 동기화는 /api/roommate POST 에서 한다).
    savePrefs({
      grade: rmProfile.student_id ? `${rmProfile.student_id}학번` : null,
      dept:  rmProfile.dept ?? null,
      zone:  null, priorities: [], gate: null, theme,
    })
