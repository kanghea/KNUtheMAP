...
// AddressStep 은 위성 이미지 + 검색 useEffect + suggestions UI 가 무겁고
// 흐름의 끝에만 등장 → next/dynamic 으로 별도 chunk 로 분리. unit/item
// 단계의 첫 hydrate 비용에서 빠져 모바일 첫 인터랙션이 빨라짐.
...
// BuildingRow / Coord 은 lazy-load 되는 AddressStep 과 공유하기 위해 _types.ts 로 분리.
...
// AddressStep 은 ./AddressStep.tsx 로 분리 — next/dynamic 으로 lazy-load.
