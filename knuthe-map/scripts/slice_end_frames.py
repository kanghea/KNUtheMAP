"""
Slice the 4-frame "end / 종료" motion played when loading completes.
Character falls into a manhole; frame 4 is mostly submerged (small).

Usage:
  python3 scripts/slice_end_frames.py <source.png> [<out_dir>]

Source: PNG/JPEG with 4 chibi characters arranged horizontally.
Each frame is auto-detected via vertical bands (variable widths).

Output: 256x256 RGBA PNGs aligned so that the **manhole rim / baseline**
        is constant across all 4 frames (so the character appears to sink
        in place rather than float). This differs from the running/tap
        scripts which align per-frame feet baseline.
"""
...
SRC = sys.argv[1] if len(sys.argv) > 1 else '/tmp/end-source.png'
...
CANVAS = 256
BASELINE_FROM_BOTTOM = 22       # 맨홀 바닥과 캔버스 하단 사이 여백
TARGET_HEIGHT_PX = 215          # 가장 큰 프레임 (1번) 의 목표 높이

WHITE_THR = 238
ALPHA_RAMP = 18
BAND_MIN_NONWHITE = 5
BAND_MIN_GAP = 8                # 인접 프레임이 합쳐지지 않을 만큼 큰 갭
...
# Per-frame metrics — note we keep GLOBAL y_max for unified baseline
metrics = []
...
for m in metrics:
    print(f"  f{m['idx']:02d}: w={m['w']:3d} h={m['h']:3d} "
          f"cx={m['cx']:6.2f} y_max={m['y_max']:4d}")

# Scale by the LARGEST frame's height (frame 1 typically) — preserves the
# size hierarchy so frame 4 stays smaller (character is submerged).
max_h = max(m['h'] for m in metrics)
scale = TARGET_HEIGHT_PX / max_h

# Unified baseline: lowest pixel across ALL frames (= manhole bottom).
# This is the key difference from per-frame baseline alignment.
global_y_max = max(m['y_max'] for m in metrics)
print(f'  scale={scale:.4f}  global_y_max={global_y_max}')
...
for m in metrics:
    ...
    # Use GLOBAL bottom so the manhole rim stays put across frames
    y0 = max(0, m['y_min'] - pad)
    y1 = min(char.shape[0], global_y_max + pad + 1)
    crop = rgba[y0:y1, x0:x1]
    cx_rel = m['cx'] - x0
    by_rel = global_y_max - y0          # ← 공통 베이스라인
