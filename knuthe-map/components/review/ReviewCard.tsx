    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24"
          fill={i <= rating ? 'var(--star-filled)' : 'var(--star-empty)'}>
...
export default function ReviewCard({ review }: { review: Review }) {
  ...
  return (
    <div className="rounded-xl p-4" style={{ border: '1px solid var(--border-primary)' }}>
      ...
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {review.room_type && (
              <span className="text-xs rounded px-1.5 py-0.5 leading-none"
                style={{ color: 'var(--text-tertiary)', border: '1px solid var(--border-secondary)' }}>
                {review.room_type}
              </span>
            )}
            {review.floor && (
              <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{review.floor}층</span>
            )}
          </div>
          <div className="flex items-center gap-2 mt-0.5">
            <Stars rating={review.rating_overall} />
            <span className="text-xs" style={{ color: 'var(--text-tertiary)' }}>{timeAgo}</span>
          </div>
        </div>
      </div>

      {/* 내용 */}
      <p className="text-sm leading-relaxed mb-3" style={{ color: 'var(--text-primary)' }}>{review.content}</p>
        ...
        <div className="flex flex-col gap-1.5 mb-3">
          {review.pros && (
            <div className="flex items-start gap-1.5">
              ...
              <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{review.pros}</span>
            </div>
          )}
          {review.cons && (
            <div className="flex items-start gap-1.5">
              <span className="text-xs font-bold text-red-400 shrink-0 mt-0.5">단점</span>
              <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{review.cons}</span>
            </div>
          )}
        </div>
      )}
      ...
      {/* 세부 평점 */}
      {catRatings.length > 0 && (
        <div className="flex flex-wrap gap-x-4 gap-y-1 pt-2.5" style={{ borderTop: '1px solid var(--border-primary)' }}>
